...
| Type | Items |
|---|---|
| Skills — plan & build | `/feature` (design a new feature → `/ingest`-ready design docs), `/project-design` (idea → elicited decisions → numbered doc set + CIA/AuthN model + `HANDOVER.md` in a fresh repo), `/ingest` (design docs → `specs/` + `tasks/`), `/implement` (autonomously build a `tasks/` backlog via worktree-isolated parallel agents, gated on a green build **and** an adversarial Codex review), `/harden` (systematically triage + fix code-review / Codex findings → verify green → commit), `/understand` (deep-review a repo → on-screen briefing + durable project memory), `/security-test` (static source-level security audit → prioritized findings report), `/devtools` (remote-debug: attach to a real or disposable Chrome over CDP and read console/network/DOM instead of screenshots) |
| Skills — ops & fleet | `/portfolio` (read-only status sweep across all `~/code-local` repos + the GitHub account, with local ↔ GitHub ↔ memory reconciliation), `/peggy` (onboard a local service into the Peggy gateway), `/peggy-doctor` (diagnose a misbehaving Peggy service against the gotcha catalog), `/daemonize` (install + verify a durable user LaunchAgent pair), `/fork-hack` (stand up a hack-fork: origin=fork, upstream push-disabled, branch `hack`), `/ship-ios-repair` (Xcode Cloud signing-repair runbook after a capability change), `/ship-ios-setup` (first-ship: ASC provisioning + Xcode Cloud so a git push auto-ships to TestFlight), `/fleet` (run a command across the Tailscale host fleet; default = plugin sync), `/host` (report this host's identity + light stats + Tailscale reachability to the other fleet hosts), `/commit` (commit + push, and PR-merge a feature branch, in one step) |
...
| Commands | `/telegram` (notify via your bot), `/gui-teardown` (quit the VS Code GUI instance a Remote Control session is hosted in + release its caffeinate), `/harden`, `/notebooklm`, `/project-design`, `/devtools` (thin command wrappers over the same-named skills) |
...
│   ├── skills/{feature,project-design,ingest,implement,harden,understand,security-test,remote-debug,portfolio,peggy,peggy-doctor,daemonize,fork-hack,ship-ios-repair,ship-ios-setup,fleet,host,commit,deep-research,notebooklm-push}/SKILL.md
│   ├── commands/{telegram,gui-teardown,harden,notebooklm,project-design,devtools}.md
